/* Neverending Story – app.js (iOS-safe narration)
   - Story Worker: POST { action, memory } -> { story_text, choices[], memory_capsule }
   - TTS Worker:  POST { text, voice }     -> { audio_base64 }
   - OpenAI TTS playback: <audio> + Blob URL (avoids iOS decodeAudioData hangs)
*/
...
function setBusy(b) {
  ...
  if (UI.replayBtn) UI.replayBtn.disabled = b; // prevent double replay while fetching
}
...
function initLocalVoices() {
  ...
  if (UI.localVoice) {
    ...
    const saved = localStorage.getItem("nes_local_voice") || "auto";
    UI.localVoice.value = saved;
  }
}
...
function localSpeak(text) {
  ...
  if (pick !== "auto") {
    ...
    if (v) u.voice = v;
  } else if (selectedLocalVoice) {
    u.voice = selectedLocalVoice;
  }
...
// ---------------- OpenAI TTS playback via <audio> Blob ----------------
...
async function fetchAndPlayOpenAITTS(ttsUrl, voice, text) {
  ...
  // Abort controller so “Replay” can’t wedge the tab
  ...
  try { json = JSON.parse(raw); }
    ...
    throw new Error("TTS worker returned non-JSON (wrong URL or Worker error).");
  }
  ...
  // IMPORTANT: await play() so we can catch iOS playback errors
...
function narrate(text) {
  ...
  // Always stop the other pipeline
  localStop();
  stopOpenAITTS();
  ...
  if (mode === "openai") {
    ...
    fetchAndPlayOpenAITTS(ttsUrl, voice, clean)
      .catch(e => {
        console.error(e);
...
async function startNew() {
  ...
  if (UI.localVoice) localStorage.setItem("nes_local_voice", UI.localVoice.value);
...
async function advance(c, choiceNumber, actionText) {
  ...
  try {
    // stop any audio immediately on new turn
    localStop();
    stopOpenAITTS();
...
function replay() {
  ...
  if (!c) return;
  // replay current story using active narration mode
  narrate(c.story || "");
}
...
function boot() {
  ...
  UI.localVoice && UI.localVoice.addEventListener("change", () => localStorage.setItem("nes_local_voice", UI.localVoice.value));
  ...
  UI.libraryBtn && (UI.libraryBtn.onclick = () => { localStop(); stopOpenAITTS(); showSetup(); setStatus("Ready."); });
