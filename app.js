/* Neverending Story – app.js (iPhone-safe narration)
   - Narration modes:
       off   = none
       local = free iPhone voice (Web Speech)
       openai= OpenAI TTS worker returning { audio_base64 }
   - Fixes iOS "hang at requesting audio" with hard timeouts + better errors
   - Pause/Resume/Stop/Replay work for BOTH narration engines
   - Wipe menu: all library / current story / last chapter (undo)
*/
...
async function fetchJsonWithTimeout(url, opts, ms, label) {
  ...
  const fetchPromise = (async () => {
    ...
    catch { throw new Error(`${label}: non-JSON response (first 200): ${txt.slice(0, 200)}`); }
...
function refreshLocalVoices() {
  ...
  // include English voices (iOS uses device voices)
...
async function fetchAndPlayOpenAi(text) {
...
// ---- unified narration control ----
function getNarrMode() {
  return (UI.narrationMode?.value || "off"); // off | local | openai
}
...
async function pauseNarration() {
  ...
  if (mode === "local") pauseLocalSpeech();
  if (mode === "openai") await pauseOpenAiAudio();
}

async function resumeNarration() {
  const mode = getNarrMode();
  if (mode === "local") resumeLocalSpeech();
  if (mode === "openai") await resumeOpenAiAudio();
}
...
function renderStory(c, text) {
  ...
  // narrate the currently displayed story
