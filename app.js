/* Neverending Story PWA – app.js (Library + Settings + iOS-safe speech)
   - Saves Worker URL once; hides it from main setup
   - Library screen lists campaigns with tap-to-load + delete
   - iOS-safe speech (prevents “repeats chapter 1”)
*/
...
function setWorkerUrl(v) {
  try { localStorage.setItem(LS_WORKER_URL, (v || "").trim()); }
  catch {}
  refreshWorkerSummary();
}
...
if (speechSupported()) speechSynthesis.onvoiceschanged = () => loadVoices();
...
function pauseSpeech() { try { if (speechSynthesis.speaking && !speechSynthesis.paused) speechSynthesis.pause(); } catch {} }
function resumeSpeech() { try { if (speechSynthesis.paused) speechSynthesis.resume(); } catch {} }
function stopSpeech() { hardStopSpeech(); }
...
async function startNew() {
  ...
  if (!workerUrl) {
    alert("Set your Worker URL in Settings first.");
    showSettings();
    return;
  }
...
function boot() {
  ...
  UI.libraryBtn && (UI.libraryBtn.onclick = () => { stopSpeech(); showLibrary(); });
  ...
  UI.pauseBtn && (UI.pauseBtn.onclick = () => {
    unlockSpeechOnce();
    try { speechSynthesis.paused ? resumeSpeech() : pauseSpeech(); } catch {}
  });

  UI.stopBtn && (UI.stopBtn.onclick = () => { unlockSpeechOnce(); stopSpeech(); });
  ...
  if (speechSupported()) setTimeout(() => { try { speechSynthesis.getVoices(); } catch {} }, 300);
}
