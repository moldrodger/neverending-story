/* Neverending Story – app.js (iOS-safe narration, improved UX)
   - Story Worker: POST { action, memory } -> { story_text, choices[], memory_capsule }
   - TTS Worker:  POST { text, voice }     -> { audio_base64 }
   - OpenAI TTS playback: <audio> + Blob URL (safe path)
   - Fixes: "canceled" UI noise, better local status, better openai status
*/
...
// Treat cancels/aborts as normal, not an error
...
async function fetchAndPlayOpenAITTS(ttsUrl, voice, text) {
  ...
  // Keep status "Narrating" until audio truly finishes
  audioEl.onended = () => {
    // only the latest token should be allowed to flip UI
    if (my === ttsToken) setStatus("Ready.");
  };
  audioEl.onerror = () => {
    if (my === ttsToken) setStatus("Ready (no audio).");
  };

  const p = audioEl.play();
  if (p && typeof p.then === "function") await p;

  // Do NOT set Ready here; onended will do it.
}
...
function narrate(text) {
  ...
  if (mode === "openai") {
    ...
    fetchAndPlayOpenAITTS(ttsUrl, voice, clean).catch(e => {
      console.error(e);
      // Abort/cancel is NORMAL (user clicked replay twice, choice advanced, etc.)
      if (isAbortLike(e)) {
        setStatus("Ready.");
      } else {
...
async function startNew() {
  ...
  try {
    ...
    // Don't force "Ready" here; narrate() may still be playing.
  } catch (e) {
    console.error(e);
