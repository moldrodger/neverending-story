/* Neverending Story – app.js (iPhone-safe narration, chunked OpenAI TTS)
   Fixes:
   - iOS hang at "requesting audio" by using res.blob() + hard timeout
   - Chapter feeling short: narrates the FULL story by chunking into multiple TTS requests
*/
...
const TTS_CHUNK_CHARS = 1100;       // safe chunk size
const TTS_TIMEOUT_MS = 20000;       // per-chunk timeout
const STORY_TIMEOUT_MS = 25000;
...
// ---------------- Timeout wrapper (works even if AbortController acts odd) ----------------
...
async function playDecodedBuffer(buf) {
  const ctx = ensureAudioContext();
...
function splitIntoChunks(text) {
  ...
  for (const p of paras) {
    const add = (cur ? (cur + "\n\n" + p) : p);
    if (add.length <= TTS_CHUNK_CHARS) {
      cur = add;
    } else {
      // if current has content, flush it
      if (cur) flush();

      // if single paragraph too big, hard slice
      if (p.length > TTS_CHUNK_CHARS) {
        let i = 0;
        while (i < p.length) {
          chunks.push(p.slice(i, i + TTS_CHUNK_CHARS).trim());
          i += TTS_CHUNK_CHARS;
        }
      } else {
        cur = p;
...
async function fetchMp3AsDecodedBuffer(ttsUrl, voice, text) {
  setStatus(`Narrating… (requesting audio, ${text.length} chars)`);

  const res = await withTimeout(fetch(ttsUrl, {
    method: "POST",
    headers: { "Content-Type": "application/json", "Accept": "audio/mpeg" },
    body: JSON.stringify({ text, voice }),
  }), TTS_TIMEOUT_MS, "TTS worker");
  ...
  // iOS stability: use blob() then arrayBuffer()
...
async function narrateOpenAi(fullText) {
  ...
  // Build queue by fetching first, then continue while playing
  playQueue = [];
  isQueuePlaying = true;

  // Fetch + play sequentially (simple and reliable)
  for (let i = 0; i < chunks.length; i++) {
    if (my !== narrToken) return;

    setStatus(`Narrating… (chunk ${i + 1}/${chunks.length})`);

    const decoded = await fetchMp3AsDecodedBuffer(ttsUrl, voice, chunks[i]);
  ...
  if (my === narrToken) setStatus("Ready.");
}
