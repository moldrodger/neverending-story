/* Neverending Story – app.js v6
   - Story worker: POST { action, memory } -> { story_text, choices[], memory_capsule }
   - Narration modes:
     off   = no narration
     local = Web Speech (free)
     openai= OpenAI TTS worker -> audio_base64 -> WebAudio decode/play (iOS safe)
   - Fixes: hangs (timeouts), “repeats chapter 1” (token gate), better status messages
*/
...
// ---------------- Local narration (Web Speech) ----------------
let localVoicesReady = false;
...
function ensureLocalVoiceList() {
  if (!speechSupported()) return;
  // force-load
  const voices = speechSynthesis.getVoices();
  if (voices && voices.length) localVoicesReady = true;
  ...
  if (UI.localVoice) {
    ...
    const en = (voices || []).filter(v => (v.lang || "").toLowerCase().startsWith("en"));
    en.forEach(v => opts.push({ value: v.name, label: `${v.name} (${v.lang})` }));
...
if (speechSupported()) {
  speechSynthesis.onvoiceschanged = () => {
    localVoicesReady = true;
    ensureLocalVoiceList();
  };
  // kick once
  setTimeout(ensureLocalVoiceList, 400);
}
...
function speakLocal(text) {
  ...
  let voice = null;
  if (pick !== "auto") {
    voice = (speechSynthesis.getVoices() || []).find(v => v.name === pick) || null;
  } else {
    voice = getBestEnglishVoice();
  }
...
// token gate prevents “old audio plays” or “repeats first chapter”
let narrToken = 0;
...
// must unlock from a user gesture on iOS
...
async function fetchAndNarrateOpenAI(text) {
  ...
  const ttsUrlRaw = (UI.ttsWorkerUrl?.value || "").trim();
  if (!ttsUrlRaw) throw new Error("Missing TTS worker URL.");

  // normalize URL (avoid accidental double slashes)
  const ttsUrl = ttsUrlRaw.endsWith("/") ? ttsUrlRaw.slice(0, -1) : ttsUrlRaw;
  ...
  const fetchPromise = fetch(ttsUrl, {
    method: "POST",
    headers: { "Content-Type": "application/json", "Accept": "application/json" },
  ...
  const res = await withTimeout(fetchPromise, 25000, "TTS request");
  if (my !== narrToken) return;

  const bodyText = await withTimeout(res.text(), 15000, "TTS response read");
  if (my !== narrToken) return;
  ...
  // decode can stall on iOS -> timeout it
...
async function narrateCurrent(c, text) {
  ...
  if (mode === "openai") {
    await fetchAndNarrateOpenAI(text);
    return;
  }
}
...
function renderStory(c, text) {
  ...
  // Fire narration (async, guarded)
  if (clean) {
    narrateCurrent(c, clean).catch((e) => {
      console.error(e);
      setStatus(`Ready (no audio: ${e.message})`);
    });
  }
}
...
async function replay() {
  ...
  const mode = getNarrMode();
  if (mode === "openai" && lastBuffer && (c.story || "") === lastNarratedText) {
    setStatus("Narrating… (replay)");
    playBuffer(lastBuffer).then(() => setStatus("Ready.")).catch(e => setStatus("Ready (no audio: " + e.message + ")"));
    return;
  }

  if (c.story) {
    setStatus("Narrating…");
    narrateCurrent(c, c.story).catch(e => setStatus("Ready (no audio: " + e.message + ")"));
  }
}
...
function boot() {
  ...
  ensureLocalVoiceList();
  if (UI.localVoice) UI.localVoice.value = savedLocalVoice;
