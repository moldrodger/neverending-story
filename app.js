/* Neverending Story – app.js (iPhone-safe OpenAI TTS via WebAudio)
   + Adds wipe controls:
     - Wipe Campaign (current only)
     - Wipe Library (everything on device)
     - Wipe to Chapter... (rollback segments)
*/
...
const UI = {
  ...
  // NEW wipe buttons
  wipeCampaignBtn: el("wipeCampaignBtn"),
  wipeToChapterBtn: el("wipeToChapterBtn"),
  wipeLibraryBtn: el("wipeLibraryBtn"),

  // legacy (if still in HTML)
  wipeBtn: el("wipeBtn"),
...
function setBusy(b) {
  [UI.startBtn, UI.loadBtn, UI.wipeBtn, UI.continueBtn, UI.wildBtn,
   UI.wipeCampaignBtn, UI.wipeToChapterBtn, UI.wipeLibraryBtn
  ].forEach(x => { if (x) x.disabled = b; });
...
function wipeLibrary() {
  if (!confirm("Wipe the entire library on this device? This cannot be undone.")) return;
  stopNarration();
  localStorage.removeItem(LS_KEY);
  localStorage.removeItem(LS_ACTIVE);
  showSetup();
  setStatus("Library wiped.");
}

function wipeCampaign() {
  const id = getActiveId();
  if (!id) return alert("No active campaign loaded.");

  const c = findCampaign(id);
  if (!c) return alert("Active campaign not found.");

  if (!confirm(`Delete campaign "${c.name}" from this device?`)) return;
...
function wipeToChapter() {
  ...
  if (total <= 1) return alert("This campaign only has 1 chapter. Nothing to roll back.");

  const pick = prompt(`Enter chapter number to keep (1-${total}).\nExample: 1 keeps only the first chapter.`);
  const n = parseInt(pick || "", 10);
  if (!n || n < 1 || n > total) return;

  if (!confirm(`Roll back "${c.name}" to chapter ${n}? Chapters ${n + 1}..${total} will be deleted.`)) return;

  stopNarration();
  c.segments = c.segments.slice(0, n);
  ...
  renderStory(c, c.story);
  renderChoices(c, c.choices);
  setStatus(`Rolled back to chapter ${n}.`);
}
...
function boot() {
  ...
  // NEW wipe wiring
  UI.wipeLibraryBtn && (UI.wipeLibraryBtn.onclick = wipeLibrary);
  UI.wipeCampaignBtn && (UI.wipeCampaignBtn.onclick = wipeCampaign);
  UI.wipeToChapterBtn && (UI.wipeToChapterBtn.onclick = wipeToChapter);

  // legacy wipe button (if still present)
  UI.wipeBtn && (UI.wipeBtn.onclick = wipeLibrary);
