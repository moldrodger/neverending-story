/* Neverending Story PWA – Clean app.js (iOS-safe speech + pause/stop + saved Worker URL)
   ...
   - Worker URL persistence:
       - saved to localStorage key LS_WORKER_URL
       - auto-filled on launch
*/

const LS_KEY = "nes_campaigns_v4";
const LS_ACTIVE = "nes_active_campaign_v4";
const LS_WORKER_URL = "nes_worker_url_v1"; // ✅ NEW: persist worker URL
...
async function startNew() {
  ...
  // ✅ SAVE worker URL so you never retype it
  try { localStorage.setItem(LS_WORKER_URL, workerUrl); } catch {}
...
function wipeAll() {
  ...
  // Note: we intentionally do NOT remove LS_WORKER_URL
  showSetup();
...
// ---------- Boot ----------
function boot() {
  // ✅ Restore Worker URL (best effort)
  // Priority: dedicated saved URL -> last campaign url
  try {
    const savedUrl = localStorage.getItem(LS_WORKER_URL);
    if (savedUrl && UI.workerUrl) UI.workerUrl.value = savedUrl;
  } catch {}

  const last = loadAll()[0];
  if (UI.workerUrl && !UI.workerUrl.value && last?.workerUrl) {
    UI.workerUrl.value = last.workerUrl;
  }

  // ✅ Save Worker URL whenever you change it (optional but helpful)
  if (UI.workerUrl) {
    UI.workerUrl.addEventListener("change", () => {
      const v = (UI.workerUrl.value || "").trim();
      if (!v) return;
      try { localStorage.setItem(LS_WORKER_URL, v); } catch {}
    });
  }
