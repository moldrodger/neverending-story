/* Neverending Story – app.js (iPhone-safe narration)
   Fix: Prevent TTS hangs/errors by narrating ONLY story text + limiting length.
*/
...
// ---------------- Fetch helper w/ timeout + better error surfacing ----------------
async function fetchJsonWithTimeout(url, opts, ms, label) {
  ...
  const fetchPromise = (async () => {
    ...
    try { json = JSON.parse(txt); }
      ...
      throw new Error(`${label}: non-JSON response (first 200): ${txt.slice(0, 200)}`);
    }

    if (!res.ok) {
      // If worker includes structured details, show them
      const extra =
        json?.status ? ` (upstream status ${json.status})` : "";
      const raw = json?.raw ? ` raw=${truncate(JSON.stringify(json.raw), 180)}` : "";
      throw new Error(`${label}: ${json?.error || `HTTP ${res.status}`}${extra}${raw}`);
    }
...
// ---------------- Narration text preparation (IMPORTANT FIX) ----------------
const OPENAI_TTS_MAX_CHARS = 1200; // safe; prevents upstream errors + huge base64

function buildNarrationText(storyText) {
  // Only narrate the story text; strip weird whitespace
  let t = String(storyText || "")
    .replace(/\r/g, "")
    .trim();

  // If it’s enormous, cap it
  if (t.length > OPENAI_TTS_MAX_CHARS) {
    t = t.slice(0, OPENAI_TTS_MAX_CHARS).trim() + "…";
  }

  return t;
}

// ---------------- Local speech synthesis (free) ----------------
let localVoices = [];
...
function pickLocalVoice(nameOrAuto) {
  ...
  if (nameOrAuto && nameOrAuto !== "auto") {
    return localVoices.find(v => v.name === nameOrAuto) || null;
  }
...
function stopLocalSpeech() { if (speechSupported()) { try { speechSynthesis.cancel(); } catch {} } }
function pauseLocalSpeech() { if (speechSupported()) { try { if (speechSynthesis.speaking && !speechSynthesis.paused) speechSynthesis.pause(); } catch {} } }
function resumeLocalSpeech() { if (speechSupported()) { try { if (speechSynthesis.paused) speechSynthesis.resume(); } catch {} } }
...
async function fetchAndPlayOpenAi(storyText) {
  ...
  // NOTE: if your chapter is huge, this text is already capped above.
  const json = await fetchJsonWithTimeout(ttsUrl, {
    method: "POST",
    headers: { "Content-Type": "application/json", "Accept": "application/json" },
...
async function narrate(storyText) {
  ...
  if (mode === "local") {
    ...
    speakLocal(buildNarrationText(clean)); // also cap for local just in case
    setStatus("Ready.");
    return;
  }
...
// Wipe menu: all / current / last chapter
function wipeMenu() {
  const c = findCampaign(getActiveId());
  const msg =
`Wipe options:
...
function boot() {
  ...
  UI.pauseBtn && (UI.pauseBtn.onclick = async () => {
    ...
    if (mode === "openai") {
      const ctx = ensureAudioContext();
      if (ctx.state === "running") await pauseOpenAiAudio();
      else await resumeOpenAiAudio();
    } else if (mode === "local") {
      if (speechSupported() && speechSynthesis.paused) resumeLocalSpeech();
